...
export namespace ExpoRouter {
  type StaticRoutes = `/` | `/(list)` | `/(tabs)` | `/(tabs)\camera` | `/_sitemap` | `/explore`;
  type DynamicRoutes<T extends string> = `/${SingleRoutePart<T>}` | `/(edit)/${SingleRoutePart<T>}` | `/(list)/${SingleRoutePart<T>}`;
  type DynamicRouteTemplate = `/(edit)/[edit]` | `/(list)/[id]` | `/[edit]` | `/[id]`;
